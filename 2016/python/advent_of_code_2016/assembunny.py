...
class AssemBunny():
    ...
    def out(self, instr):
        _, v = instr.split(' ')
        return int(v) if is_digit(v) else self.registers[v]

    def parse_instructions(self, instr, steps=10**999):
        idx = 0
        signal = []
        s = 0
        while idx < len(instr) and s < steps:
            s += 1
            i = instr[idx]
            ...
            if i.startswith('cpy'):
                ...
                signal.append(self.out(i))
            elif i.startswith('jnz'):
                _, reg, offset = i.split(' ')

                if str.isalpha(offset):
